...
NOTE WHAT THIS MODULE DOES *NOT* COVER ON THIS SITE. Etsy's listing pages are
gated by DataDome, not reCAPTCHA, and DataDome's solvable form is a
`DataDomeSliderTask` — a different task type, requiring the caller's own
proxy, which the engines do not currently drive. Etsy's own refusal is
detected in `product_parser.detect_page_state` (`t=bv` blocked, `t=fe` a
solvable challenge) and reported there; nothing in this file is reached by
it. That is deliberate rather than an omission: over five measured attempts
on five fresh residential exits, two `t=fe` pages both came back
ERROR_CAPTCHA_UNSOLVABLE, and the access path that works on this site —
the Scraping Browser API — needed no solve at all.
